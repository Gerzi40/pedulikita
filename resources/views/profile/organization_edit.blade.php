...
@section('content')
    ...
    {{-- <form action="{{ route('organization.profile.update') }}" method="post" enctype="multipart/form-data">
        ...
        <div>
            Founded At:
            <input type="date" name="founded_at" class="border-2 border-solid" value="{{ old('founded_at', $user->organization->founded_at) }}" required />
            @error('founded_at')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        <div>
            Instagram:
            <input type="text" name="instagram" class="border-2 border-solid" value="{{ old('instagram', $user->organization->instagram) }}" required />
            @error('instagram')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        <div>
            Phone:
            <input type="text" name="phone" class="border-2 border-solid" value="{{ old('phone', $user->organization->phone) }}" required />
            @error('phone')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            Description:
            <input type="text" name="description" class="border-2 border-solid" value="{{ old('description', $user->organization->description) }}" required />
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <form action="{{ route('organization.profile.update') }}" method="post" enctype="multipart/form-data"
            ...
            <div>
                ...
                <input type="date" name="founded_at" class="w-full border rounded-md px-4 py-2 text-sm"
                    value="{{ old('founded_at', $user->organization->founded_at) }}" required />
                @error('founded_at')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="text" name="instagram" class="w-full border rounded-md px-4 py-2 text-sm"
                        value="{{ old('instagram', $user->organization->instagram) }}" required />
                    @error('instagram')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="text" name="phone" class="w-full border rounded-md px-4 py-2 text-sm"
                        value="{{ old('phone', $user->organization->phone) }}" required />
                    @error('phone')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="email" name="email" class="w-full border rounded-md px-4 py-2 text-sm disabled:bg-gray-200 disabled:text-gray-500 disabled:cursor-not-allowed"
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Keterangan <span
                        class="text-red-500">*</span></label>
                <textarea name="description" rows="4" class="w-full border rounded-md px-4 py-2 text-sm" required>{{ old('description', $user->organization->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="relative w-48 h-48 rounded-md overflow-hidden cursor-pointer group">
                    <!-- Preview Image -->
                    ...
                    <div
                        class="absolute inset-0 bg-black/40 flex items-center justify-center text-white text-sm font-medium opacity-0 group-hover:opacity-100 transition">
                        Ubah Foto
                    </div>
                </div>
    ...
    $('#province').on('change', function() {
        ...
        if (provinceId) {
            $.get(`/provinces/${provinceId}/cities`, function(data) {
                ...
                data.forEach(city => options += `<option value="${city.id}">${city.name}</option>`);
                $('#city').html(options);
            });
        } else {
